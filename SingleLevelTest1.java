...
class Student extends Person {
	...
	Student(char gender, String name, int age, int rollNumber, String collegeName, String stream, int totalMarks) {
...
class Employee extends Student {
	...
	Employee(char gender, String name, int age,int rollNumber, String collegeName, String stream, int totalMarks, int empno,String companyName, String designation,float salary) {
		super(gender,name,age,rollNumber,collegeName,stream,totalMarks); //REUSE THE ctor OF THE PARENT
		...
		if(Validator.checkNumber(empno, 1, 500)) {
			...
			RuntimeException runtimeRollNumberEx = new RuntimeException("Invalid Roll Number : "+empno);
			throw runtimeRollNumberEx;
		}
		...
		if(Validator.checkString(companyName)) {
			...
			RuntimeException runtimeNameEx = new RuntimeException("Invalid collegeName : "+companyName);
			throw runtimeNameEx;
		}
		...
		if(Validator.checkString(designation)) {
			...
			RuntimeException runtimeNameEx = new RuntimeException("Invalid stream : "+designation);
			throw runtimeNameEx;
		}
		

		if(Validator.checkNumber(salary, 1, 1000)) {
			this.salary = salary;			
		}
		else {
			RuntimeException runtimeMarksEx = new RuntimeException("Invalid Marks : "+salary);
			throw runtimeMarksEx;
		}
